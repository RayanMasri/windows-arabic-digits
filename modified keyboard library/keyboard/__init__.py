...
def copy_recopied():
    #print('Copied recopy')
    pyperclip.copy(recopied)

copy_timer = Timer(timer_time, copy_recopied)
...
def key_to_scan_codes(key, error_if_missing=True):
    ...
    try:
        ...
        #print(list(_os_keyboard.map_name(normalized)))
...
def send(hotkey, do_press=True, do_release=True, recopy=None):
    #if code in ARABIC_NUMBERS:
        #pyperclip.copy(ARABIC_NUMBERS[code])
        #hotkey = 'ctrl+v'
        #s1 = pyperclip.paste()
        #print(s1)
        #return
    ...
    for step in parsed:
        ...
        if do_press:
            for scan_codes in step:
                #print(scan_codes[0])
                #print(scan_codes[0])
                if scan_codes[0] in ARABIC_NUMBERS:
                    copied = pyperclip.paste()
                    pyperclip.copy(ARABIC_NUMBERS[scan_codes[0]])                    
                    #print(copied)
                    #print(scan_codes[0])
                    #print(ARABIC_NUMBERS[scan_codes[0]])
                    return send('ctrl+v', recopy=copied)
    ...
    if recopy:
        if recopy not in list(ARABIC_NUMBERS.values()):            
            recopied = recopy
            #print('Set recopy variable to non arabic digit')
        if copy_timer:
            #print('Cancelled previous timer')
            copy_timer.cancel()
        #print('Created new timer and started it')
        copy_timer = Timer(timer_time, copy_recopied) 
        copy_timer.start()
        #pyperclip.copy(recopy)
...
def remap_key(src, dst):
    ...
    def handler(event):
        ...
        if event.event_type == KEY_DOWN:            
            #print(f'Remap key pressed: {dst}')
            press(dst)
        else:
            #print(f'Remap key released: {dst}')
            release(dst)
        return False
