import keyboard, json, ctypes, time, sys
...
if 'English' not in get_keyboard_language():
    for number in english_numbers:
        keyboard.remap_key(number, arabic_numbers[int(number)])

def on_quit_callback(systray):    
    keyboard.unhook_all()
    exitter()
state = 'Enabled' if 'English' not in get_keyboard_language() else 'Disabled'
systray = SysTrayIcon("logo.ico", f"""Arabic Digits ({state})""", on_quit=on_quit_callback)
systray.start()
...
def change_language():
    ...
    if 'English' in language:
        systray.update(hover_text='Arabic Digits (Disabled)')
        unhook_all()
        toast.show_toast(
            "Disabled arabic digits",
            "Disabled arabic digits",
            icon_path='',
            duration = 3,
            threaded = False
        ...
        systray.update(hover_text='Arabic Digits (Enabled)')
        toast.show_toast(
            "Enabled arabic digits",
            "Enabled arabic digits",
            icon_path='',
            duration = 3,
            threaded = False
...
def unhook_all():
    keyboard.unhook_all()
    keyboard.add_hotkey('alt+shift', change_language, timeout=5)
keyboard.add_hotkey('alt+shift', change_language, timeout=5)
